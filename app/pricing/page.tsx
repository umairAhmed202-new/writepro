...
import { Check, BadgeCheck, Loader2 } from "lucide-react";
...
const PricingPage: React.FC = () => {
  ...
  const [userSubscription, setUserSubscription] = useState<boolean>(false); // Replace with real data
  ...
  const plans: PricingPlan[] = [
    {
      ...
      features: ["10,000 Words/Month", "50+ Content Templates", "1 Month of History"],
      cta: "Get Started",
      popular: false,
    ...
    },
    {
      ...
      features: ["1,000,000 Words/Month", "50+ Template Access", "1 Year of History"],
      cta: userSubscription ? "Active Plan" : "Upgrade Now",
      popular: true,
      isCurrent: userSubscription,
    },
  ];
  ...
  return (
    <div className="bg-gray-50 dark:bg-neutral-950 min-h-screen">
      {/* Header with current page highlighting */}
      ...
      <div className="pt-24 pb-12 px-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-3xl">
          <h2 className="text-center font-bold text-3xl m-7 dark:text-white">Upgrade With Monthly Plan</h2>

          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 sm:items-center md:gap-8">
            {plans.map((plan) => (
              <div
                key={plan.name}
                className={`rounded-2xl bg-white dark:bg-neutral-800 border ${
                  plan.popular ? "border-blue-500 dark:border-violet-500 shadow-lg" : "border-gray-200 dark:border-neutral-700"
                } p-6 shadow-sm sm:px-8 lg:p-12 relative`}
              >
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 px-3 py-1 bg-gradient-to-r from-blue-600 to-violet-600 text-white text-xs font-medium rounded-full flex items-center">
                ...
                <div className="text-center">
                  ...
                  <p className="mt-2 sm:mt-4">
                    <strong className="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">{plan.price}</strong>
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{plan.period}</span>
                  </p>
                </div>
                ...
                <ul className="mt-6 space-y-2">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-center gap-1">
                      <Check className="size-5 text-blue-500 dark:text-violet-500 flex-shrink-0" />
                ...
                {plan.name === "Free" ? (
                  <Link href="/dashboard">
                    <Button className="w-full rounded-full mt-5 p-6 font-medium">{plan.cta}</Button>
                  </Link>
                ) : (
                  <Link href="/dashboard/billing">
                    <Button
                      className="w-full rounded-full mt-5 p-6 font-medium"
